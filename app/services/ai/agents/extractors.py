...
4. **格式规范**：
   - **数组(List)** 类型的字段（如 tags, priorities）：如果没有提取到信息，必须返回 **[]** (空数组)，禁止返回 null。
   - **字典(Dict)** 类型的字段（如 strength）：如果没有提取到信息，必须返回 **{{}}** (空字典)，禁止返回 null。
   - 其他字段（String/Number/Boolean）：如果没有信息，请返回 null。
